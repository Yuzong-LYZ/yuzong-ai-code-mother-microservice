package com.yuzong.yuzongaicodemother.utils;
...
public class WebScreenshotUtils {
    ...
    /**
     * 初始化 Chrome 浏览器驱动【这个不用自己写。复制即可，有需要改的照着改就行】
     */
        ...
        try {
            ...
            wait.until(webDriver ->
                    ((JavascriptExecutor) webDriver).executeScript("return document.readyState")
                            .equals("complete") // 为complete则表示页面加载完成
            );
            // 额外等待一段时间，确保动态内容加载完成 2秒
            Thread.sleep(2000);
            log.info("页面加载完成");
        } catch (Exception e) {
